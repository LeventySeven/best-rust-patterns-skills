...
**Invariant hole — deserialization bypasses your constructor.** `Email::new` only runs when *you* call it. `serde_json::from_str`, a DB row mapper, or `#[derive(Deserialize)]` reconstruct the newtype field-by-field, so an invalid value round-trips straight past validation. Bind validation to the deserialization boundary so the validated constructor is the *only* way in:

```rust
// OpenAI Codex — codex-rs/protocol/src/agent_path.rs (AgentPath newtype)
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize, JsonSchema, TS)]
#[serde(try_from = "String", into = "String")]
pub struct AgentPath(String);
// serde now calls TryFrom<String> (your validation) on deserialize,
// and Into<String> on serialize — invalid values are rejected at the edge.
```
...
| Validate only in `new()` | serde/DB deserialization rebuilds an invalid value | `#[serde(try_from, into)]` to validate at the boundary |
