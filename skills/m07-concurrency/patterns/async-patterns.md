...
### RAII Task Cancellation (Drop aborts the task)

The `CancellationToken` pattern above is *cooperative* — the task must poll `cancelled()`. The complementary pattern is *structural*: tie a task's lifetime to a struct by wrapping its `JoinHandle` in a newtype whose `Drop` aborts it. When the parent struct drops, the task is aborted — no leak, no orphan.

```rust
// (Conductor — tauri-plugins/plugins/http/src/reqwest_cookie_store.rs:126-133)
#[derive(Debug)]
...
**Why:** every spawned task whose lifetime is logically owned by a struct (a connection's read loop, a per-connection background sync) gets cleaned up for free at the call sites — no explicit teardown code.

**Footgun this fixes:** a bare `tokio::task::JoinHandle` does **not** cancel its task when dropped — the task keeps running detached. (Contrast `tokio::task::JoinSet`, where dropping the *set* aborts all members, and some framework `Task` types that are drop-cancelling.) If you `let _ = tokio::spawn(...)` and drop the handle expecting cleanup, you have a leak.

**Limit:** `abort()` is not instantaneous — it only takes effect at the next `.await`. A task stuck in a blocking section (sync sleep, blocking I/O not behind `spawn_blocking`) ignores the abort until it yields. For bounded-time cancellation use a `CancellationToken` (above) instead.
