...
### Concrete alternative to `Arc<Mutex<T>>`: own the state in one task

The table says "message passing" beats `Arc<Mutex<T>>` everywhere — here is what that actually looks like. Give one task **exclusive ownership** of the state and let every other task talk to it over an `mpsc` mailbox. All mutation runs through a single `fn receive(&mut self, msg)` that the owning task calls serially, so concurrent access is impossible by construction — zero locks, zero deadlocks.

```rust
// (LakeSail — sail-execution/src/driver/actor/core.rs:18-148; field names verbatim)
struct Driver {
    // each field owned EXCLUSIVELY by the actor task — no Arc, no Mutex, no RwLock
    worker_pool: WorkerPool,
    job_scheduler: JobScheduler,
    task_assigner: TaskAssigner,
    stream_manager: StreamManager,
    task_runner: TaskRunner,
    task_sequences: HashMap<TaskId, u64>, // key/value types illustrative
}

impl Driver {
    // called serially by the single actor task draining the mailbox
    fn receive(&mut self, msg: DriverEvent) {
        match msg { /* mutate self.* directly; &mut access is exclusive */ }
    }
}
// one task: `while let Some(msg) = rx.recv().await { driver.receive(msg); }`
```

**Gotcha:** *every* reader must also go through the actor, even read-only queries. LakeSail handles this with an `ObserveState { observer }` message that pushes a callback the actor invokes synchronously inside `receive`, keeping the actor the sole owner.

Reach for `Arc<Mutex<T>>` when you have a few shared cells and contention is low; reach for the actor when one "central authority" (scheduler, registry, session manager) needs serialized access to a lot of state.
