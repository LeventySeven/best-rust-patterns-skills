...
### Making an Error Enum `Clone` (Arc-wrap the non-Clone source)

`io::Error` is not `Clone`, so `#[derive(Clone)]` on an error enum that carries one fails to compile. Wrap it in `Arc` via a manual `From` so the whole tree is clone-able — essential when an error fans out to multiple observers (log + retry + user-facing):
...
The `From<io::Error>` impl Arc-wraps; `#[from]` still works for the already-Clone sub-errors. Trade-off: `Arc<E>` loses mutability of the inner error, so add call-chain context with anyhow in app code rather than mutating the wrapped error. (tantivy, `src/error.rs:55-118`.)
...
### Auto-Capture Source Location on a Custom Error Enum

anyhow's `.context()` gives a human message but no `file:line`. For a typed error enum that must stay machine-inspectable AND carry debug location, add a `Context(String, Box<Self>)` variant and a macro that stamps the call site:
...
Wrapping with `Context(String, Box<DataFusionError>)` preserves the underlying variant (still downcastable/matchable), unlike erasing to a string. The `context!` macro formats `"{desc} at {file}:{line}"`. Cost: one allocation per `.context` call; deep chains format slowly. (DataFusion, `datafusion/common/src/error.rs:146-147`.)
