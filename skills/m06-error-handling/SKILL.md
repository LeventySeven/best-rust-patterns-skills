...
The table above says "don't `.unwrap()` in production" — enforce it mechanically instead of by review. Add to the workspace `Cargo.toml` so every member crate inherits the denials:
...
`allow_attributes = "deny"` forces explicit `#[expect(...)]` over `#[allow(...)]`, so every suppression is visible. Tests that legitimately need `unwrap` annotate the call with `#[expect(clippy::unwrap_used)]`. (LakeSail, `Cargo.toml:17-23`.)
