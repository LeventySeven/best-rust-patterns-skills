...
### Controlling Variance on Your Own Types

The rules above cover the built-in reference types. When you write a generic
newtype/handle that does *not* actually own a `T` (e.g. an id, a typed handle,
a cursor), you pick its variance and drop-check behavior with a `PhantomData`
marker. Production example — Zed's `gpui` typed entity handle
...
| Marker | Variance over `T` | dropck checks `T`? |
|--------|-------------------|--------------------|
| `PhantomData<T>` | covariant | yes (asserts ownership) |
| `PhantomData<fn() -> T>` | covariant | no |
| `PhantomData<fn(T) -> T>` | **invariant** | no *(Zed's choice)* |
| `PhantomData<fn(T)>` | contravariant | no |

`fn(T) -> T` puts `T` in both the argument (contravariant) and the return
(covariant) position, which compose to **invariant** — so `Entity<T>` neither
coerces across different `T` nor (thanks to the `fn` indirection) makes the drop
checker believe it owns a `T`, while staying `Send + Sync`. Invariance is the
safe default for an opaque handle. If you instead want covariant lifetime
coercion without owning a `T`, use `PhantomData<fn() -> T>`.
