...
## Static OR Dynamic — You Can Have Both

The Core Question forces a choice — but a single trait can offer **both** paths.
Use a Generic Associated Type for the zero-cost static path, and a `dynamic()`
method that erases to a trait object for the heterogeneous-storage path. Callers
who know their types pay nothing; callers who need a `Vec` of mixed types opt in.

```rust
// LanceDB — lance-tokenizer/src/analyzer.rs. The GAT means the erased form is
// a SEPARATE trait (BoxableTokenizer), bridged back to Tokenizer here:
...
impl<T: Tokenizer> TextAnalyzerBuilder<T> {
    // static path: keeps the concrete type, fully monomorphized, no vtable
    fn filter<F: TokenFilter>(self, f: F) -> TextAnalyzerBuilder<F::Tokenizer<T>> {
        TextAnalyzerBuilder { tokenizer: f.transform(self.tokenizer) }
    }
    // escape hatch: erase to Box<dyn> only when you need heterogeneous storage
    fn dynamic(self) -> TextAnalyzerBuilder {
        TextAnalyzerBuilder { tokenizer: Box::new(self.tokenizer) }
    }
}
```

- Default path: `filter(f)` returns a concrete `TextAnalyzerBuilder<F::Tokenizer<T>>`
  — full static dispatch, inlinable, no vtable.
- `dynamic()` erases to `Box<dyn BoxableTokenizer>` only when you actually need
  heterogeneous storage (e.g. a runtime-configured pipeline in a `Vec`).
- Both forms implement `Tokenizer`, so downstream code is written once.

Reach for this when your Decision Guide answer is "mostly generics, but one
collection needs mixed types" — instead of forcing the whole API to `dyn`.
