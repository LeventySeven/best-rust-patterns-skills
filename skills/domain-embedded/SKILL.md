...
| `f64::ln()` / `sqrt()` under no_std | Float transcendentals are std-only | `libm::log(x)`, `libm::sqrt(x)` |

---

## Code Pattern: no_std Float Math

`f64::ln`, `sqrt`, `sin`, etc. live in `std::f64` and disappear under `#![no_std]`. Reach for the `libm` crate (pure-Rust software `<math.h>`, no allocator, no platform deps):

```rust
#![no_std]
extern crate alloc;

// f64::ln(x) is unavailable under no_std — use libm::log (natural log):
let uniform: f64 = self.prng.next_u64() as f64 / core::u64::MAX as f64;
(-libm::log(uniform) * libm::log(M as f64).recip()) as usize
```

`libm::log` is `ln`. Tradeoff: ~3-5x slower than hardware `f64::ln()` (no FMA/vectorization), so gate it `#[cfg(not(feature = "std"))]` and use `f64::ln()` for std builds. (turbopuffer — hnsw_rs `src/hnsw/hnsw_const.rs`.)
