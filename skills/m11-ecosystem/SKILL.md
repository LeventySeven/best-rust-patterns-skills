...
| `[workspace.dependencies]` | One version per dep across all crates |

**Centralizing versions (the mechanism behind "Unify in workspace"):** declare every shared dependency once in the workspace-root `Cargo.toml`, then reference it from each member crate with no version. A single bump propagates to every member, and you cannot accidentally pull two versions of the same crate (the "Duplicate types" error). Zed runs 236 crates this way — versions live only at the root.

```toml
# workspace root Cargo.toml (Zed — /tmp/zed/Cargo.toml)
...
| One fat shared crate | Every member pays the heaviest dep's compile | Two-level split: zero-dep base + framework-aware crate |

**Two-level split (LakeSail):** keep pure domain/spec types in a `*-common` crate with zero external deps; put framework-coupled shared types (e.g. DataFusion-aware) in a separate `*-common-datafusion` crate. Light crates (a parser, CLI) depend only on the base and never trigger the multi-million-line framework compile.

```toml
# workspace root Cargo.toml (LakeSail — /tmp/sail/Cargo.toml)
[workspace]
members = ["crates/*"]
resolver = "2"
# crates/sail-common        -> no DataFusion dep
# crates/sail-common-datafusion -> DataFusion-aware shared types
# sail-sql-parser depends only on sail-common (skips the DF compile)
```
