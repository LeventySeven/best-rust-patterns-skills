...
### 3b. Clone Just to Move a Field Out

The 'borrow instead of clone' fix doesn't apply when you genuinely need to *own* a field reached through `&mut self` — the borrow checker forbids moving out of a borrow, so beginners reach for `.clone()`. Use `std::mem::take` instead: it swaps in `T::default()` and returns the original in one operation, no clone, no second allocation. Works for any `T: Default` (`Vec`, `String`, `HashMap`, `BTreeMap`).

```rust
// ANTI-PATTERN: clone a field you're about to overwrite anyway
fn drain(&mut self) -> Vec<Job> {
    let jobs = self.queue.clone();   // wasteful copy
    self.queue = Vec::new();
    jobs
}

// BETTER: take leaves Default in place, returns the original (zero copy)
fn drain(&mut self) -> Vec<Job> {
    std::mem::take(&mut self.queue)  // one op, no allocation
}
```

Seen in Zed's editor for drain-by-replacement on a struct field (`crates/editor/src/git.rs:2509`):
...
### 15b. Reflexive Arc<Mutex<T>> for All Shared State

Reaching for `Arc<Mutex<T>>` (or `RwLock`) on every field that more than one task touches is the most common Rust concurrency smell. If only **one** task ever mutates the state, you don't need a lock at all — give that task exclusive ownership of the data and have other tasks *send it messages*. This removes lock contention, lock-ordering deadlocks, and the lock-across-await trap (#14) by construction.

```rust
// ANTI-PATTERN: every consumer contends on one lock, mutations scattered
let state = Arc::new(Mutex::new(WorkerPool::new()));
// task A: state.lock().unwrap().assign(job);
// task B: state.lock().unwrap().rebalance();   // lock-order bugs lurk here

// BETTER: one owner task holds the state by value; others send messages
let (tx, mut rx) = tokio::sync::mpsc::channel::<Msg>(8);
tokio::spawn(async move {
    let mut pool = WorkerPool::new();          // owned, no Arc/Mutex
    while let Some(msg) = rx.recv().await {
        match msg { Msg::Assign(j) => pool.assign(j), Msg::Rebalance => pool.rebalance() }
    }
});
```

LakeSail's actor runtime (`crates/sail-execution/src/driver/actor/core.rs:18-148`) owns all mutable fields (`WorkerPool`, `JobScheduler`, `TaskAssigner`, `StreamManager`, `TaskRunner`, `task_sequences: HashMap`) directly inside the actor struct with no `Arc`/`Mutex`/`RwLock`; every mutation routes through a `fn receive(&mut self, ...)` called serially by the single actor task — even read-only queries go through it (via a `DriverEvent::ObserveState { observer }` callback). Reach for a full actor abstraction only when you have many such state machines; for one, the bare channel above is enough. (Trace to m07-concurrency / m12-lifecycle.)
